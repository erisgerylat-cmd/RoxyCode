...
优势是上手成本低、配置可读、适合教学和中文业务团队落地。MCP 已补齐 stdio、HTTP、Streamable HTTP、SSE、WS/WebSocket 与 OAuth PKCE 的基础闭环；劣势是当前还没有 Claude Code 那样完整的 marketplace、系统级凭据存储、复杂 Hook policy 和远程生态治理。后续可以在这个骨架上补齐 keychain、资源订阅、重连和插件市场能力。
