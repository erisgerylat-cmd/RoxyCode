...
export function createBuiltinCommands(options: BuiltinCommandFactoryOptions): CommandDefinition[] {
  ...
  return [
    ...
    },
    {
      ...
      source: 'builtin',
      type: 'local',
      usage: isZh ? '/character [role|info|list|create|packages|install|uninstall|update|validate|pack|export]' : '/character [name|info|list|create|packages|install|uninstall|update|validate|pack|export]',
      examples: ['/character', '/character roxy', '/character info', '/character create my-waifu-dev --package', '/character packages', '/character install ./roxy-sensei.roxychar', '/character pack ./.roxycode/characters/roxy-sensei', '/character export current --roxychar'],
      subcommands: [
        ...
        { name: 'packages', description: isZh ? '列出已安装角色包' : 'List installed character packages' },
        { name: 'search', description: isZh ? '搜索 RoxyStore 角色包商城' : 'Search the RoxyStore marketplace', needsInput: true },
        { name: 'install', description: isZh ? '安装角色包（本地路径或商城包名）' : 'Install a character package (local path or store name)', needsInput: true },
        { name: 'uninstall', description: isZh ? '卸载角色包' : 'Uninstall a character package', needsInput: true },
        { name: 'update', description: isZh ? '更新角色包' : 'Update a character package', needsInput: true },
        { name: 'validate', description: isZh ? '校验角色包' : 'Validate a character package', needsInput: true },
        { name: 'pack', description: isZh ? '打包 .roxychar 角色包' : 'Pack a .roxychar archive', needsInput: true },
        { name: 'export', description: isZh ? '导出内置或当前角色' : 'Export a built-in or current character', needsInput: true },
      ],
      handler: args => handleCharacterCommand(args, options.characterManager, options.configManager),
    },
    {
      name: 'aesthetic',
